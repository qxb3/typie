use ratatui::{crossterm::style::Color, layout::{Constraint, Layout, Rect}, style::Stylize, text::Text, Frame};
 ...
 \__/[\_:  /   | ;.__/ [___]'.__.'
      \__.'   [__|
        ...
        frame.render_widget(
            Text::from(name),
            center(
                Constraint::Length(name.lines().map(|l| l.len()).max().unwrap() as u16),
